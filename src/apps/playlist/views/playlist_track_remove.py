...
KINO_ID = "playlist-track-remove"

class PlaylistTrackRemoveView(BaseAPIView):
    """
    プレイリストトラック削除APIクラス
    """
    ...
    def delete(self, request, playlist_id: str, track_id: str, *args, **kwargs):
        """
        DELETEリクエストを受け付ける。
        Method: DELETE
        Args:
        ...
        """
        try:
            return self.playlist_track_remove(request, playlist_id, track_id, *args, **kwargs)
    ...
    def playlist_track_remove(self, request, playlist_id: str, track_id: str, *args, **kwargs):
        """
        プレイリストトラック削除処理
        Args:
            request:  HTTPリクエスト
        ...
        log_output_by_msg_id(
            log_id="MSGI003",
            params=[KINO_ID, f"playlist_id: {playlist_id}, track_id: {track_id}"],
            logger_name=LOG_METHOD.APPLICATION.value,
        )
        
        # 3. サービス実行(トラック追加)
        remove_track = self.playlist_service.remove_playlist_track(
            date_now=date_now,
            kino_id=KINO_ID,
            user=request.user,
        ...
        # 4. レスポンス作成
        res_serializer = CustomPlaylistTrackResponseSerializer(remove_track)
        ...
        log_output_by_msg_id(
            log_id="MSGI004",
            params=[KINO_ID, f"Deleted playlist_id: {playlist_id}, track_id: {track_id}"],
            logger_name=LOG_METHOD.APPLICATION.value,
        )
