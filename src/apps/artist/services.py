...
class ArtistService:
    ...
    # ------------------------------------------------------------------
    # アーティスト一覧取得
    def list_artist(self, date_now: datetime, kino_id: str, user: M_User, name=None, tag_ids=None):
        """
    ...
    def detail_artist(self, date_now: datetime, kino_id: str, user: M_User, artist_id):
        ...
        try:
            ...
            artist = T_Artist.objects.filter(
                id=artist_id,
                user=user,
                deleted_at__isnull=True
            ).select_related(
    ...
    # ------------------------------------------------------------------
    # アーティスト更新
    def update_artist(self, date_now: datetime, kino_id: str, user: M_User, artist_id, validated_data):
        """アーティストを新規登録する"""
        # 1. 対象の取得（存在チェック）
        try:
    ...
    def refresh_artist(self, date_now: datetime, kino_id: str, user: M_User, artist_instance: T_Artist):
        """
    ...
    def refresh_artists(self, date_now: datetime, kino_id: str, user: M_User, artist_queryset):
        """
    ...
    def search_artist(self, user: M_User, query, limit):
        ...
        spotify_raw_list = self.spotify_service.fetch_search_artists(query=query, limit=limit)
